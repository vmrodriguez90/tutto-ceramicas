...
<div class="newsletter-footer js-newsletter-footer p-relative">
    <form method="post" action="/winnie-pooh" onsubmit="$(this).attr('action', '');" data-store="newsletter-form">
        <div class="input-append full-width p-relative m-none-xs">
          <input type="text" class="m-none newsletter-footer-input footer-input border-box form-control {% if not mobile %} span8 {% endif %} {% if newsletter_contact_error %} input-error {% endif %}" onfocus="if (this.value == '{{ "Email" | translate }}') {this.value = '';}" onblur="if (this.value == '') {this.value = '{{ "Email" | translate }}';}" value='{{ "Email" | translate }}' name="email"  aria-label='{{ "Email" | translate }}' >
        ...
        <span class="btn btn-footer {% if not mobile %} p-all-half {% endif %} newsletter-footer-btn submit-icon">
