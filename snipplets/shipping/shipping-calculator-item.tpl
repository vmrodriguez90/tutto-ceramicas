...
<li class="js-shipping-list-item radio-button-item list-item" data-store="shipping-calculator-item-{{ option.code }}">
	<label class="js-shipping-radio radio-button shipping-option" data-loop="shipping-radio-{{loop.index}}" data-shipping-type="{% if pickup %}pickup{% else %}delivery{% endif %}">
		<input 
			...
			<span class="radio-button-label">
				...
				<div class="row-fluid radio-button-text">
					<div class="span9 col-xs-9-custom pull-left-xs">
						<div class="shipping-method-name {% if option.payment_rules or option.time or option.suboptions is not empty %}m-bottom-quarter{% endif %}">
							{{option.short_name}} {% if option.method == 'branch' %}<span class="m-left-quarter">{{ option.extra.extra }}</span>{% endif %}
						</div>
						{% if option.time %}
                            <div class="font-body opacity-80 {% if option.suboptions is not empty or option.payment_rules %}m-bottom-quarter{% endif %}">
